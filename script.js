...
(() => {
  ...
  const initReel = () => {
    ...
    /* Точки лежат внутри ленты на всех трёх страницах — координата 709
       в .reel__dots считается от неё. Стоит вынести ряд соседом, как было
       на странице проекта и о нас, и отсчёт пойдёт от секции: точки уезжают
       на середину центрального кадра. Обрезки бояться нечего — лента
       обрезается рамкой .reel__viewport уровнем ниже. */
    const dots = [...reel.querySelectorAll(".reel__dots .dot")];
